...
/**
 * Class ***`CreateCategoriesDeCompteTable`***
 *
 * A migration class for creating the "categories_de_compte" table with UUID primary key and timestamps.
 *
 * @package ***`\Database\Migrations\CreateCategoriesDeCompteTable`***
 */
class CreateCategoriesDeCompteTable extends Migration
{
    ...
    public function up(): void
    {
        ...
        try {
            ...
            Schema::create('categories_de_compte', function (Blueprint $table) {
                // Define a UUID primary key for the 'categories_de_compte' table
                $this->uuidPrimaryKey($table);

                // Define a unique string column for the categories_de_compte code
                $table->string('code')->unique()
                    ->comment('The unique code of the categorie de compte');
                                
                // Define a unique string column for the categories_de_compte name
                $table->string('name')->unique()
                    ->comment('The unique name of the categorie de compte');
            ...
            throw new \Core\Utils\Exceptions\DatabaseMigrationException(
                message: 'Failed to migrate "categories_de_compte" table: ' . $exception->getMessage(),
                previous: $exception
            );
        }
    }
    ...
    public function down(): void
    {
        ...
        try {
            // Drop the "categories_de_compte" table if it exists
            Schema::dropIfExists('categories_de_compte');
            ...
            // Handle the exception (e.g., logging, notification, etc.)
            throw new \Core\Utils\Exceptions\DatabaseMigrationException(
                message: 'Failed to drop "categories_de_compte" table: ' . $exception->getMessage(),
                previous: $exception
            );
        }
    }
