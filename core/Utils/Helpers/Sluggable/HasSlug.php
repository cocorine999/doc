...
namespace Core\Utils\Helpers\Sluggable;

use Illuminate\Database\Eloquent\Model;
use Core\Utils\Helpers\Sluggable\SlugOptions;
...
trait HasSlug
{
    ...
    protected function generateAndSetSlug(Model $model)
    {
        if ($this->getSlugOptions()->iSlugOnEmpty() && !$this->getAttribute($this->getSlugOptions()->getSlugAttribute())) {
            $slug = $this->generateUniqueSlug(implode($this->getSlugOptions()->getSource()), $model);
     ...
     * @return string
     */
    protected function generateUniqueSlug(string $sourceAttributes, Model $model): string
    {
        $slug = $this->getSlugOptions()->generateUniqueSlugWithSuffix($model->getAttribute($sourceAttributes), $model);
