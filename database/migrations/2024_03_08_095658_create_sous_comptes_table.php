...
/**
 * Class ***`CreateSousComptesTable`***
 *
 * A migration class for creating the "sous_comptes" table with UUID primary key and timestamps.
 *
 * @package ***`\Database\Migrations\CreateSousComptesTable`***
 */
class CreateSousComptesTable extends Migration
{
    ...
    public function up(): void
    {
        ...
        try {
            ...
            Schema::create('sous_comptes', function (Blueprint $table) {
                // Define a UUID primary key for the 'sous_comptes' table
                $this->uuidPrimaryKey($table);

                // Define a unique string column for the sous_comptes code
                $table->string('code')->unique()
                    ->comment('The unique code of the classe de compte');
                                
                // Define a unique string column for the sous_comptes name
            ...
            throw new \Core\Utils\Exceptions\DatabaseMigrationException(
                message: 'Failed to migrate "sous_comptes" table: ' . $exception->getMessage(),
                previous: $exception
            );
        }
    }
    ...
    public function down(): void
    {
        ...
        try {
            // Drop the "sous_comptes" table if it exists
            ...
            // Handle the exception (e.g., logging, notification, etc.)
            throw new \Core\Utils\Exceptions\DatabaseMigrationException(
                message: 'Failed to drop "sous_comptes" table: ' . $exception->getMessage(),
                previous: $exception
            );
        }
    }
