 ...
 * - `grantAccess($permissionIds, $data = [])`: Grant access to certain permissions for the model.
...
trait HasPermissions
{
     ...
     * @return self
     */
    public function grantAccess($permissionIds, array $data = []): self
    {
    ...
    public function revokeAccess($permissions, array $data = [], string $filter = 'id'): self
    {
        ...
        foreach ($this->permissions()->wherePivotIn("permission_id", $permissionIds)->get() as $key => $permission) {
