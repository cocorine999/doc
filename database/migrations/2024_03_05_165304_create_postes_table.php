...
/**
 * Class ***`CreatePostesTable`***
 *
 * A migration class for creating the "postes" table with UUID primary key and timestamps.
 *
 * @package ***`\Database\Migrations\CreatePostesTable`***
 */
class CreatePostesTable extends Migration
{
    ...
    public function up(): void
    {
        ...
        try {
            ...
            Schema::create('postes', function (Blueprint $table) {
                // Define a UUID primary key for the 'postes' table
                $this->uuidPrimaryKey($table);
                
                // Define a unique string column for the postes name
                $table->string('name')->unique()->comment('The unique name of the postes');
            ...
            throw new \Core\Utils\Exceptions\DatabaseMigrationException(
                message: 'Failed to migrate "postes" table: ' . $exception->getMessage(),
                previous: $exception
            );
        }
    }
    ...
    public function down(): void
    {
        ...
        try {
            ...
            // Handle the exception (e.g., logging, notification, etc.)
            throw new \Core\Utils\Exceptions\DatabaseMigrationException(
                message: 'Failed to drop "postes" table: ' . $exception->getMessage(),
                previous: $exception
            );
        }
    }
