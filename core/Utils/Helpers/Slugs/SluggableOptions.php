...
namespace Core\Utils\Helpers\Slugs;

use Closure;
use Core\Utils\Helpers\Sluggable\SlugColumnsMapping;

/**
 * Class SluggableOptions
 *
 * The `SluggableOptions` class provides options and configuration settings for the Sluggable trait.
 * The Sluggable trait is used to generate slugs for model instances based on certain attributes.
 *
 * @package Core\Utils\Helpers\Slugs
 */
class SluggableOptions
{
     ...
     * @var bool
     */
    protected bool $generateSlugOnEmpty = false;
     ...
     */
    public static function create(): SluggableOptions
    {
        return new static();
    }
     ...
     */
    public function generateUniqueSlugWithSuffix(string $baseSlug, int $suffix): string
    {
        if ($suffix > 0) {
        ...
        if ($this->isSlugReserved($slugWithSuffix) || $this->isSlugExistsInDatabase($slugWithSuffix)) {
            if ($suffix >= $this->maxAttempts) {
                throw new \RuntimeException("Maximum number of attempts reached to generate a unique slug.");
            }
            return $this->generateUniqueSlugWithSuffix($baseSlug, $suffix + 1);
        }
     ...
     * @return bool
     */
    private function isSlugReserved(string $slug): bool
    {
     ...
     * @return bool
     */
    private function isSlugExistsInDatabase(string $slug): bool
    {
        // Implement logic to check if the given slug already exists in the database.
        // Return true if the slug exists, otherwise return false.
        // Example (assuming Eloquent is used):
