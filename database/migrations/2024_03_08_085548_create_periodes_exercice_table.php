...
/**
 * Class ***`CreatePeriodesExerciceTable`***
 *
 * A migration class for creating the "periodes_exercice" table with UUID primary key and timestamps.
 *
 * @package ***`\Database\Migrations\CreatePeriodesExerciceTable`***
 */
class CreatePeriodesExerciceTable extends Migration
{
    ...
    public function up(): void
    {
        ...
        try {
            ...
            Schema::create('periodes_exercice', function (Blueprint $table) {
                // Define a UUID primary key for the 'periodes_exercice' table
                $this->uuidPrimaryKey($table);
                
                // Define a unique string column for the periodes_exercice name
                $table->string('name')->unique()->comment('The unique name of the periode name');
                ...
                $this->compositeKeys(table: $table, keys: ['name', 'date_debut_periode', 'date_fin_periode', 'status', 'can_be_delete']);
            ...
            throw new \Core\Utils\Exceptions\DatabaseMigrationException(
                message: 'Failed to migrate "periodes_exercice" table: ' . $exception->getMessage(),
                previous: $exception
            );
        }
    }
    ...
    public function down(): void
    {
        ...
        try {
            // Drop the "periodes_exercice" table if it exists
            Schema::dropIfExists('periodes_exercice');
            ...
            // Handle the exception (e.g., logging, notification, etc.)
            throw new \Core\Utils\Exceptions\DatabaseMigrationException(
                message: 'Failed to drop "periodes_exercice" table: ' . $exception->getMessage(),
                previous: $exception
            );
        }
    }
