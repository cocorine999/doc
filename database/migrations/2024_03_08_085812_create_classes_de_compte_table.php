...
/**
 * Class ***`CreateClassesDeCompteTable`***
 *
 * A migration class for creating the "classes_de_compte" table with UUID primary key and timestamps.
 *
 * @package ***`\Database\Migrations\CreateClassesDeCompteTable`***
 */
class CreateClassesDeCompteTable extends Migration
{
    ...
    public function up(): void
    {
        ...
        try {
            ...
            Schema::create('classes_de_compte', function (Blueprint $table) {
                // Define a UUID primary key for the 'classes_de_compte' table
                $this->uuidPrimaryKey($table);

                // Define a unique string column for the classes_de_compte code
                $table->string('code')->unique()
                    ->comment('The unique code of the classe de compte');
                                
                // Define a unique string column for the classes_de_compte name
            ...
            throw new \Core\Utils\Exceptions\DatabaseMigrationException(
                message: 'Failed to migrate "classes_de_compte" table: ' . $exception->getMessage(),
                previous: $exception
            );
        }
    }
    ...
    public function down(): void
    {
        ...
        try {
            // Drop the "classes_de_compte" table if it exists
            Schema::dropIfExists('classes_de_compte');
            ...
            // Handle the exception (e.g., logging, notification, etc.)
            throw new \Core\Utils\Exceptions\DatabaseMigrationException(
                message: 'Failed to drop "classes_de_compte" table: ' . $exception->getMessage(),
                previous: $exception
            );
        }
    }
